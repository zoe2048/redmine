        ...
        if by_type == 'version' or by_type == 'category':
            ...
            for qxcd in cf_qxcd:
                bugsobj = redmine.issue.filter(
                    project_id=pro_id,
                    status_id='*',
                    cf_1=qxcd  # cf_1: 其中1为自定义字段‘缺陷程度’的id值，id值可使用redmine.get_custom_field()获取
                )
                bugs_num = len(bugsobj)
                yield qxcd, bugs_num
        else:
...
def get_issue_by_one_ver(by_type, pro_id, tracker_id=1):
    """
    获取某个特定版本内的bug数据
    :param by_type: 类型
    :param pro_id: 项目标识
...
if __name__ == '__main__':
    ...
    if s == '2':
        ...
        for r in results:
            ...
            print(type(r))
    if s == '3':
        result1 = get_id('field')
        result2 = get_id('tracker')
        print(result1)
        print(result2)
